...
/
// Need to rewrite this a bit to make it work with BeagleBoneBlue

void VL53L1X::startMeasurement(uint8_t offset)
{
  (void) offset ;

// Need to configure the distance sensor

   cpi2c_writeRegisters(_i2cport, 0x0001, 64, &configBlock[0]) ;
   cpi2c_writeRegisters(_i2cport, 0x0041, 64, &configBlock[64]) ;
   cpi2c_writeRegisters(_i2cport, 0x0081,  7, &configBlock[128]) ;
}
